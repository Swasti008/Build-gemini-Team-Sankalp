...
export function AnimeNavBar({ items, className, defaultActive = "Home" }: NavBarProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (heroSection) {
        ...
        setIsVisible(scrollY < 20 || scrollY < heroBottom - 100)
      } else {
  ...
  // Track section in view for hash URLs
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      if (visible) {
        const name = items.find(i => i.url === `#${visible.target.id}`)?.name
  ...
  return (
    <div className={cn("fixed top-20 left-0 right-0 z-[30] pointer-events-none", className)}>
          ...
          {items.map((item) => {
            ...
            return (
              <a
                key={item.name}
                href={item.url}
                onClick={(e) => { handleClick(e, item); setActiveTab(item.name) }}
                  ...
                  >
                    <div className="relative w-12 h-12">
                      <motion.div 
                        ...
                        animate={ hoveredTab ? { scale: [1, 1.1, 1], rotate: [0, -5, 5, 0], transition: { duration: 0.5, ease: "easeInOut" } } : { y: [0, -3, 0], transition: { duration: 2, repeat: Infinity, ease: "easeInOut" } } }
                      >
                        <motion.div className="absolute w-2 h-2 bg-black rounded-full" animate={ hoveredTab ? { scaleY: [1, 0.2, 1], transition: { duration: 0.2, times: [0, 0.5, 1] } } : {} } style={{ left: '25%', top: '40%' }} />
                        <motion.div className="absolute w-2 h-2 bg-black rounded-full" animate={ hoveredTab ? { scaleY: [1, 0.2, 1], transition: { duration: 0.2, times: [0, 0.5, 1] } } : {} } style={{ right: '25%', top: '40%' }} />
                        ...
                        <motion.div className="absolute w-4 h-2 border-b-2 border-black rounded-full" animate={ hoveredTab ? { scaleY: 1.5, y: -1 } : { scaleY: 1, y: 0 } } style={{ left: '30%', top: '60%' }} />
                        <AnimatePresence>
                          {hoveredTab && (
                            <>
                      ...
                      <motion.div className="absolute -bottom-1 left-1/2 w-4 h-4 -translate-x-1/2" animate={ hoveredTab ? { y: [0, -4, 0], transition: { duration: 0.3, repeat: Infinity, repeatType: "reverse" } } : { y: [0, 2, 0], transition: { duration: 1, repeat: Infinity, ease: "easeInOut", delay: 0.5 } } }>
